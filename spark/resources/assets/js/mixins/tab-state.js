            ...
            if(parameters.length > 1) {
                for (var i =0 ; i < parameters.length; i++){
                    ...
                    if(parameters[i].charAt(0) === "#") {
                        parameters[i] = parameters[0]+"/"+parameters[i];
                    }

                    var tab = $(`${this.pushStateSelector} a[href="#${parameters[i]}"][data-toggle="tab"]`);

                    if (tab.length > 0) {
                        tab.tab('show');
                    }

                    this.broadcastTabChange(parameters[i], parameters);
                }
            }else {
