 ...
 * specified has access to the proposal request whose ID is indicated in the URL
 *
...
class ProposalRequestQueryString
{
    ...
    public function handle($request, Closure $next, $guard = null)
    {
        ...
        if (! $hasQueryStringParams) {
            throw new NotAuthorizedException('You are not authorized to view this proposal request.');
        }

        //  Pole for a user that has the supplied ID and hash AND has access to the proposal request
        $user = $this->userRepo->findUsingProposalRequestAndUserHash(
            $request->get($userKey),
