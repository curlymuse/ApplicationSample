...
class ChangeOrderSetProcessedByMe extends MyMailable
{
    ...
    public function build()
    {
        if ($this->changeOrder->initiated_by_party == 'hotel') {
            if ($this->changeOrder->contract->is_client_owned) {
                $receivingParty = $this->changeOrder->contract->proposal->proposalRequest->client->name;
            } else {
        ...
        $data = [
            ...
            'link'  => route(
                sprintf(
                    '%ss.contracts.show',
                    $this->changeOrder->initiated_by_party
                ),
                [
        ...
        return $this->view('emails.multiple.change-order.processed-by-me')
            ->text('emails.multiple.change-order.processed-by-me_plain')
